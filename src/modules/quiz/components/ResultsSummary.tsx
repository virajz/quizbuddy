...
export function ResultsSummary() {
    const { quiz, answers, reset, generate } = useQuizContext() as any;
    if (!quiz) return null;
        ...
        <Card>
            ...
            <CardContent className="space-y-6">
                ...
                <div className="flex gap-3">
                    <Button size="sm" onClick={() => generate({ topic: quiz.topic, level: quiz.level, locale: quiz.locale, seed: quiz.id.slice(0, 8) })}>
                        <RefreshCcw className="h-4 w-4 mr-1" /> Retake similar quiz
                    </Button>
